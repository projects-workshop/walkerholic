...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(value = "SELECT DISTINCT p FROM Product p LEFT JOIN FETCH p.productImages i LEFT JOIN FETCH i.productImages LEFT JOIN FETCH p.user u LEFT JOIN FETCH p.reviews r LEFT JOIN FETCH r.user s WHERE u.id=:id",
    ...
    @Query(value = "SELECT DISTINCT p FROM Product p LEFT JOIN FETCH p.productImages i LEFT JOIN FETCH i.productImages LEFT JOIN FETCH p.reviewInfo WHERE p.name LIKE %:keyword%",
        countQuery = "SELECT count(DISTINCT p) FROM Product p")
    Page<Product> findAllByKeyword(Pageable pageable, String keyword);

    @Query(value = "SELECT DISTINCT p FROM Product p LEFT JOIN FETCH p.productImages i LEFT JOIN FETCH i.productImages LEFT JOIN FETCH p.reviewInfo LEFT JOIN FETCH p.user u WHERE u.id=:id AND p.category=:category AND p.name LIKE %:keyword% ",
        countQuery = "SELECT count(DISTINCT p) FROM Product p")
    Page<Product> findAllBySellerAndCategory(Pageable pageable, Integer id, Category category,
        String keyword);

    @Query(value = "SELECT DISTINCT p FROM Product p LEFT JOIN FETCH p.productImages i LEFT JOIN FETCH i.productImages LEFT JOIN FETCH p.reviewInfo LEFT JOIN FETCH p.user u WHERE u.id=:id AND p.name LIKE %:keyword%",
        countQuery = "SELECT count(DISTINCT p) FROM Product p")
