...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(value = "SELECT DISTINCT p FROM Product p LEFT JOIN FETCH p.productInfo LEFT JOIN FETCH p.productImages i LEFT JOIN FETCH i.productImages LEFT JOIN FETCH p.reviewInfo LEFT JOIN FETCH p.user u LEFT JOIN FETCH u.userInfo WHERE p.id=:id ")
    ...
    @Query(value = "SELECT DISTINCT p FROM Product p LEFT JOIN FETCH p.productInfo d LEFT JOIN FETCH p.productImages i LEFT JOIN FETCH i.productImages LEFT JOIN FETCH p.reviewInfo LEFT JOIN FETCH p.user u LEFT JOIN FETCH u.userInfo WHERE u.id=:id AND d.category=:category AND d.name LIKE %:keyword% ",
        countQuery = "SELECT count(DISTINCT p) FROM Product p")
    Page<Product> findAllBySellerAndCategory(Pageable pageable, Integer id, Category category, String keyword);

    @Query(value = "SELECT DISTINCT p FROM Product p LEFT JOIN FETCH p.productInfo d LEFT JOIN FETCH p.productImages i LEFT JOIN FETCH i.productImages LEFT JOIN FETCH p.reviewInfo LEFT JOIN FETCH p.user u LEFT JOIN FETCH u.userInfo WHERE u.id=:id AND d.name LIKE %:keyword%",
        countQuery = "SELECT count(DISTINCT p) FROM Product p")
